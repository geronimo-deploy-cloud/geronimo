...
app = FastAPI(
    title=PROJECT_NAME,
    description="Iris classification API with monitoring",
    version="1.0.0",
    lifespan=lifespan,
)
...
class IrisFeatures(BaseModel):
    """Iris flower features for prediction."""
    sepal_length: float
    sepal_width: float
    petal_length: float
    petal_width: float
...
def predict(request: PredictRequest):
    ...
    3. postprocess() - format response with species name
    """
    try:
        endpoint = get_endpoint()
        features_dict = request.features.model_dump()
        result = endpoint.handle({"features": features_dict})
        
        # Map prediction to species name
        species_map = {0: "setosa", 1: "versicolor", 2: "virginica"}
        species = species_map.get(result, "unknown")
